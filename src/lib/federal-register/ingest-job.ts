 ...
 */
export async function runIngestJob(supabase: SupabaseClient): Promise<JobResult> {
  ...
  try {
    const documents = await fetchAllDocuments({ publicationDateGte: isoDateDaysAgo(INGEST_TRAILING_WINDOW_DAYS) });
    ...
    await finishRun(supabase, runId, { status: "failure", newCount, updatedCount, errorMessage });
